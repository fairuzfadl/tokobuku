...
import { ArrowLeft, BookOpen, Globe, MapPin, Building2 } from 'lucide-react';
import { formatRupiah } from '@/utils/currency';

export default function AdminPublisherShow({ publisher }) {
  return (
    <AdminLayout title={`Penerbit: ${publisher.name}`}>
      <div className="mb-5">
        <Link href={route('admin.publishers.index')} className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft className="h-4 w-4" /> Kembali ke Daftar Penerbit
        </Link>
      </div>
        ...
        <div className="lg:col-span-1 space-y-4">
            ...
            <div className="space-y-3 text-sm">
              {publisher.city && (
                <div className="flex items-center gap-2 py-2 border-b border-gray-50">
                  ...
                  <span className="text-gray-700">{publisher.city}</span>
                </div>
              )}
              {publisher.website && (
                <div className="flex items-center gap-2 py-2 border-b border-gray-50">
                  <Globe className="h-4 w-4 text-gray-400 flex-shrink-0" />
                  <a href={publisher.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-xs truncate">
                    {publisher.website}
                  </a>
                </div>
              )}
              <div className="flex justify-between items-center py-2 border-b border-gray-50">
                ...
                <span className="font-bold text-orange-600">{publisher.books?.length ?? 0}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-gray-500">Ditambahkan</span>
                <span className="text-gray-700 text-xs">{new Date(publisher.created_at).toLocaleDateString('id-ID')}</span>
              </div>
            </div>
          </div>
          ...
          {publisher.description && (
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{publisher.description}</p>
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b">
              <h3 className="font-semibold text-gray-800">
                Buku dari {publisher.name}
                <span className="ml-2 text-sm font-normal text-gray-400">({publisher.books?.length ?? 0} judul)</span>
              </h3>
            </div>
            ...
            {publisher.books?.length > 0 ? (
              ...
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm">Belum ada buku untuk penerbit ini</p>
              </div>
            )}
          </div>
        </div>
